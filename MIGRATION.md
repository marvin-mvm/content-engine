...
1. `poll_video.py` doesn't pass `--prompt` to produce.py → Content Matrix col F gets the local mp4 path instead of the generation prompt (which sits unused in `job["params"]["prompt"]`). Fix in Phase A4.
...
|---|---|---|---|---|
| F1 | **Publishing (Blotato)** | Per-job: `acme-blotato publish` from the M6 package, per-platform captions, write `published_posts` row in Supabase | 🟡 Medium | Straightforward per Operator. ⚠️ Verify first whether `blotato.py` accepts local files or needs a public/uploaded media URL; add upload step if needed. Add `threads`/`x`/`fb` platform variants to `copy.py` (bug #2). |
